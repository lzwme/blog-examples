...
export interface WDCOptions {
  ...
  /** 按文件后缀过滤。默认为 .dat */
  ext?: string;
  /** 按文件大小过滤。0 表示不过滤。默认为 10 * 1024 (10kB) */
  minFileSize?: number;
  /** 是否过滤缩略图。默认为 true */
  ignoreThumb?: boolean;
  /** 是否过滤相同的文件（名称不同） */
...
export class WxDatConvert {
  ...
  getDatFiles(srcDir: string, destDir?: string, ext = '.dat') {
    ...
    this.logger.info('源文件目录:', color.cyan(srcDir));
    this.logger.info('输出文件目录:', color.cyan(destDir));
  ...
  async convert(filepath: string, destpath: string) {
    ...
    if (this.options.minFileSize && stats.size < this.options.minFileSize) return result;
    ...
    const destName = `${dateFormat('yyyy-MM-dd_hhmmss', result.stats.ctime)}_${basename(result.dest)}`;

    result.dest = resolve(destDir, destName);

    this.logger.debug('ext:', ext, v);
    if (existsSync(result.dest)) {
      this.logger.info('文件已存在：', color.cyan(result.dest));
    } else  {
      ...
      if (converted) {
        ...
        // 修改文件属性
        promises.utimes(result.dest, stats.atime, stats.mtime);
        this.logger.info('已写入文件：', color.green(result.dest));
      }
    }
  ...
  async start(options: WDCOptions = { src: process.cwd() }) {
    ...
    const result = await concurrency(
      ...
      Math.max(1, Number(this.options.parallel) || 50)
    );
