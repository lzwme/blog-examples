...
class Blacklist
{
    ...
    public static function parse($list, $all = false)
    {
        if ($all) {
            ...
            if (1 == $list['off']) {self::black($list, $all);}
        }
    }
    ...
    public static function shell($match, $list, $all, $type = '')
    {
        ...
        switch ($type) {
            ...
            case '0':
                ...
                $val = parse_url($val, PHP_URL_HOST); //取出来源域名

                if (in_array($val, $match['val']) == ('1' == $match['match'])) {self::shell($match, $list, $all);}
                break;
            //目标域名
            case '1':
                $val = isset($_REQUEST['url']) ? $_REQUEST['url'] : '';
                $val = parse_url($val, PHP_URL_HOST); //取出目标域名
                if (in_array($val, $match['val']) == ('1' == $match['match'])) {self::shell($match, $list, $all);}
                break;
            //浏览器标识
            case '2':
                $val = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : ''; //取出浏览器标识
                if (preg_match('/' . implode("|", $match['val']) . "/i", $val) == ('1' == $match['match'])) {self::shell($match, $list, $all);}
                break;
            //客户IP
            case '3':
    ...
    public static function black($list, $all)
    {
        ...
        foreach ($match as $key) {if (1 == $key['off']) {self::shell($key, $list, $all, $key['type']);}}
    }
...
class ValidateCode
{
    ...
    private $__codeString; //验证字符串母体
    private $__resource; //图片资源
    ...
    private function __createCode($type)
    {
        switch ($type) {
            case 1:
