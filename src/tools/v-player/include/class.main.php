...
class GlobalBase
{
    ...
    public static function check()
    {
        ...
        // if (!filter_has_var(INPUT_COOKIE, 'log')) {
        //     $url = "//server.xymov.net/api/?type=xyplay_log&url=" . filter_input(INPUT_SERVER, 'HTTP_HOST');
        //     if (!self::curl($url)) {file_get_contents($url);}
        //     setcookie('log', '1', time() + 3600 * 24, '/');
        // }
    }
...
class Blacklist
{
    ...
    public static function black($list)
    {
        ...
        foreach ($match as $key) {if (1 == $key['off'] && preg_match("{" . $key['for'] . "}i", $_SERVER['PHP_SELF'])) {self::shell($key, $list, $key['type']);}}
    }
