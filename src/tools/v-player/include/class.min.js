...
function PostError(data, success, error) {
  $.ajax({
    url: 'https://lzw.me/x/v-player/api.php',
    data: data,
    dataType: 'jsonp',
    jsonp: 'cb',
    type: 'post',
...
!(function () {
  ...
  function fnErrorTrap(msg, url, line) {
    PostError(
      { type: 'xyplay_error', msg: msg, url: url, line: line },
      function (data) {
        document.write("<div style='margin-top:90px;text-align:center;'><font color='#ff0000'>程序运行异常！" + data['m']) +
          "&nbsp;<a onclick='location.replace(location.href);' href='javascript:;'>返回</a>";
      },
      function () {
        document.write(
          "<div style='margin-top:90px;text-align:center;'><font color='#ff0000' >程序运行异常，未成功上报服务器，请联系技术人员处理！'&nbsp;<a onclick='location.replace(location.href);' href='javascript:;'>返回</a></font></div>"
        );
      }
    );
  }
