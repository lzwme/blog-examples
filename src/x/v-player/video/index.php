...
if ("" != $url) {
    ...
    if (stristr($url, '.mp4') !== false || stristr($url, '.m3u8') !== false || stristr($url, '.flv') !== false) {
        ...
        # 27盘资源处理
    } elseif (stristr($url, '27pan') !== false || explode("/", parse_url($url)['path'])[1] == 'share') {
        ...
        # 360看看处理
    } elseif (stristr($url, 'v.360kan.com') !== false) {
        ...
        # 搜索资源站
    } elseif (stristr($url, 'http://') !== false || stristr($url, 'https://') !== false) {
