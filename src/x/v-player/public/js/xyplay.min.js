...
!(function () {
  ...
  function updateTopHref(search) {
    if (self === self.top) return;

    try {
      var turl = new URL(self.top.location.href);
      turl.search = search || location.search; // `?flag=${this.flag}&type=${this.type}&id=${this.id}&wd=_`; // &num=
      self.top.history.replaceState({}, '', turl.toString());
    } catch (_e) {}
  }
  ...
  xyplayer.prototype = {
    ...
    success: function (code, data, get) {
      ...
      switch (code) {
        ...
        case 3:
          ...
          this.reline();
          var url =
            'undefined' !== typeof data.url ? data.url : this.playlist_array.length >= this.part ? this.playlist_array[this.part] : '';
          //如果非后台获取,调用云播放
          if (!get) {
            this.log('获取云资源成功');
            if (this.onflag !== 'line') {
      ...
      var search = `?flag=${this.flag}&id=${this.id}&wd=_`;
      if (this.id) updateTopHref(search);
    },
          ...
          switch (num) {
            ...
            default:
              ...
              if ($this.tp === 'url') {
                ...
                $this.url = $this.playlist_array[$this.part];
    ...
    AutoPlay: function (type, url, play) {
      ...
      if (url && typeof txtCopy2 === 'function') {
        if (txtCopy2(url))
          h5Utils.alert({
            position: 'top-end',
            icon: 'success',
    ...
    xyplay: function (word, live, myplay) {
      ...
      if (myplay === '自定义') {
        ...
        var url =
          ...
          '&logo_off=' +
          this.play.play.all.logo_off +
          '&logo_style=' +
          this.play.play.all.logo_style +
          '&ver=' +
          (this.play.play.all.ver === '1' ? 'x' : '') +
          '&p2pinfo=' +
          this.play.play.all.p2pinfo +
          '&posterr=' +
          this.play.off.posterr +
          '&seektime=' +
          this.play.play.all.seektime +
          '&danmaku=' +
          this.play.play.all.danmaku;
      }
    ...
    reline: function () {
      ...
      if (this.play.off.yun === '1' && (this.onflag === 'yun' || this.playlist_array.length > 0 || this.lsplay('ext', this.url))) {
        ...
        if (this.onflag === 'yun') {
          $(this.line).append(
            '<li class="click_work"  style="' + this.play.style.line_on + '" type="line" num="yun">' + this.play.all.yun_title + '√</li>'
          );
        } else {
      ...
      if (
        this.play.off.jx === '1' &&
        this.tp !== 'wd' &&
        this.play.style.line_show === '1' &&
        'undefined' !== typeof this.play.define.jx_url &&
        this.play.define.jx_url.length > 0
      ) {
        for (var i = 0, len = this.play.define.jx_url.length; i < len; i++) {
          ...
          if (this.onflag === 'line' && Number(this.myurl) === i) {
            $(this.line).append(
              '<li class="click_work" style="' + this.play.style.line_on + '" type="line" num="' + i + '"> ' + name + '√</li>'
            );
          } else {
    ...
    relive: function () {
      ...
      for (var key in $this.play.define.live_url) {
        if (key === this.live_num && 'live' === this.onflag) {
          $(this.plive).append(
            '<li class="click_work " style="' + this.play.style.line_on + '" type="live" num="' + key + '">' + key + '√</li>'
          );
        } else {
    ...
    reflaglist: function (flag) {
      ...
      for (var i = 0, len = this.list_array.length; i < len; i++) {
        ...
        if (i === this.pflag) {
          $(this.flaglist).append(
            '<li   class="click_work" type="flag" val="' +
              this.list_array[i].flag +
              '" num="' +
              i +
              '" style="' +
              this.play.style.play_on +
              '">' +
              flag_name +
              '</li>'
          );
          ...
          $(this.flaglist).append(
            '<li   class="click_work" type="flag" val="' + this.list_array[i].flag + '" num="' + i + '">' + flag_name + '</li>'
          );
        }
      }
    },
          ...
          if (!this.play.video_match || !pat.test(array[0])) {
            ...
            if (i === Number(this.part)) {
              $(this.playlist).append(
                '<li class="click_work" type="list" num="' + i + '" style="' + this.play.style.play_on + '">' + array[0] + '√</li>'
              );
            } else {
    ...
    /*   内部函数：输出 高亮调试信息  */
    onVer: function () {
      var message = 'XyPlayer X3';
      if (typeof console === 'object') {
        console.log(
          '\n %c XyPlayer X3 %c http://lzw.me \n\n',
